...
def search_for_kmers(kmers, nmax=1000, threshold=0.5, fn='./SILVA_138.2_SSURef_NR99_tax_silva_filtered.fasta'):
    ...
    for km in kmers:
        ...
        cs = [x[0] for x in km]
        os = [x[1] for x in km]
        rel_os = [os[i] - os[i-1] for i in range(1,k)]
        for i in range(k):
            re_str += cs[i]
            if i < k-1 and rel_os[i] > 1:
                re_str += '.{' + str(rel_os[i]-1) + '}'
    ...
    for i in SeqIO.parse(fn, 'fasta'):
        ...
        # find kmers
        found = np.array([r.search(s) != None for r in kmer_regexes], int) #find_gapped(s, kmer_chars, kmer_offs)
        kmer_counts += found
        if np.sum(found) == 0:
            print('none found:', total_seqs)
            seqs_none_found += 1
