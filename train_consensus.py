...
def make_gapped_dataset(seqs_gt, seqs_mut):
    ...
    for i in tqdm(range(len(seqs_gt))):
        s = seqs_gt[i]
...
def find_biomarker_kmers(klen, min_freq_SILVA):
    ...
    for h,i in tqdm(enumerate(SeqIO.parse(fasta_f,'fasta'))):
...
if __name__ == '__main__':
    ...
    with print_time('finding gapped kmers'):
        X_gap, y = make_gapped_dataset(gt, mut)
    ...
    # make full dataset + train
    with print_time('training'):
        X = np.stack([np.sum(X_gap, axis=-1), X_up, X_bm], axis=-1)
